...
int main()
{
	...
	printf("\n -> enqueuing: 13(prio=0), 'a'(prio=5), 8(prio=2), \"sysprog\"(prio=1)\n");
	//prio_q_enqueue(q, create_int(13));
	
	
	prio_q_enqueue(q, create_char('a'));
	
	prio_q_enqueue(q, create_int(8));
	prio_q_enqueue(q, create_string("sysprog"));
